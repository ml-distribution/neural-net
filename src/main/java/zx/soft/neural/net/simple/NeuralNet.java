package zx.soft.neural.net.simple;
...
public class NeuralNet implements Serializable {
	...
	private static final long serialVersionUID = -5579323988354686994L;
	...
	Vector<Neuron> neurons; // set of neurons in this neural network
	Vector<Edge> edges; // set of edges in this neural network
	...
	**/
	private void MLPNetworkSetup(int[] numInLayer) {
		...
		for (int i = 0; i < layerNo - 1; i++) {
			// and layer numbering j
			for (int j = 0; j < numInLayer[i]; j++) {
				parent = neurons.get(neuronsIndex[i][j]); // get neuron at this location
				for (int k = 0; k < numInLayer[i + 1]; k++) {
					child = neurons.get(neuronsIndex[i + 1][k]);
	...
	**/
	public void printWeight() {
		for (int i = 0; i < edges.size(); i++) {
			System.out.print("Weight of edge " + i + ": " + edges.get(i).getWeight() + "  ");
		}
	}

	/**
	* run the network given an array of attributes from an example
	* @param Sample example contains the input attributes
	...
	**/
	private void runNetwork(Sample example) {
		// Step 1
		Neuron input;
		double x;
		for (int j = 0; j < inputNum; j++) {
			input = neurons.get(neuronsIndex[0][j]);
		...
		for (int i = 1; i < layerNo; i++) {
			for (int j = 0; j < neuronsIndex[i].length; j++) {
				n = neurons.elementAt(neuronsIndex[i][j]);
				n.calValue();
			}
		}
	}
	...
	**/
	private void trainSingle(Sample example) {
		runNetwork(example);
	...
	**/
	public int testSingle(Sample example) {
		Neuron output;
		runNetwork(example);
		output = neurons.elementAt(neuronsIndex[layerNo - 1][0]);
	...
	**/
	...
	public void reportAccuracy(DataSet trainingSet, DataSet evaluationSet, int epoch, int n, FileWriter out)
			throws IOException {
	...
	**/
	public void iterativeTrain(DataSet trainingSet, DataSet evaluationSet, int epochNum, FileWriter out)
			throws IOException {
	...
	**/
	public void backPropagation(Sample example) {
		Neuron output;
		Neuron n;
		Edge e;
		double targetVal;
