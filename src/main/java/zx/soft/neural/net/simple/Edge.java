...
/**
 * Class Edge for the neural network Each edge has an assoicated weight that changes along the way
 * 
 * @author wanggang
 *
 */
...
public class Edge implements Serializable {
	...
	private static final long serialVersionUID = 7131886554498475750L;

	/*********** Variables **************/
	private double weight; // the weight associated with this edge
	private final Neuron source; // the neuron it is connected to in layer i
	private final Neuron dest; // the neuron it is connected to in layer i+1
	private final int id; // identifier for the edge
	private final int mode; // the mode to use for update rule
	...
	**/
	public void updateDeltaWeight(double errterm) {
		Neuron s = getSource();
		deltaWeight = deltaWeight + learningRate * (1 - momentum) * errterm * s.getValue() + momentum * deltaWeightOld;
	}
