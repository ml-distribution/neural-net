package zx.soft.ann.dvincent1337;
...
public class BackPropCost implements CostFunction {
	 ...
	 */
	public BackPropCost(DoubleMatrix setX, DoubleMatrix setY, int[] setTopology, double setLambda) {
	...
	public Tuple<Double, DoubleMatrix> evaluateCost(DoubleMatrix input) {
		...
		for (int i = (num_layers - 2); i >= 1; i--) {
			...
			newDelta.copy((Theta.get(i).transpose().mmul(delta.firstElement().transpose())).mul(
					NeuralNetwork.sigmoidGradiant(
							DoubleMatrix.concatHorizontally(DoubleMatrix.ones(z.get(i - 1).getRows()), z.get(i - 1)))
							.transpose()).transpose());//matlab: newDelta = ( ( ((Theta{i})')*(delta{p-1})' ).*sigmoidGradient([ones(size(z{p},1),1) z{p}])'
